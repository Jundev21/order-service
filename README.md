...
주문이 생성이되면 주문 정보가 저장되고 그 후에 카프카로 재고 정보를 보낸다

여기서 만약 주문정보가 저장은 됐지만 다른 문제가생겨 카프카로 재고 정보를 못보낸다면? 

주문정보를 롤백이안되고 저장이된다 DB 는 원자성이긴하지만 DB 랑 카프카랑은 서로 다른 영역이기 때문이다. 

그래서 주문 정보 저장이랑 카프카 전송 을 하나의 트렌젠션으로 묶기위해서는 OUTBOX 패턴을 사용한다

OUTBOX 패턴이뭐냐

주문서가 저장되면 OUTBOX 테이블에 주문서가 저장되고 주문서의 결과 처리 상태를 표시한다 
그러면 스케쥴러를 통해서 주문서의 결과 처리상태들을 조회하고 PENDING 인 상품이 있으면 카프카로 전송하게된다. 